...
export default function JobsPage() {
  ...
  const [modal, setModal] = useState({
    ...
    primaryLabel: "OK", primaryAction: null, secondaryLabel: null, secondaryAction: null,
  });

  const showModal = (options) => setModal({ isOpen: true, title: "", message: "", type: "info", primaryLabel: "OK", primaryAction: null, secondaryLabel: null, secondaryAction: null, ...options });
  ...
  const handleApply = async (jobId) => {
    if (appliedJobs.has(jobId)) {
      showModal({ title: "Already Applied", message: "You have already applied to this job.", type: "info" });
      return;
    }
    try {
      ...
      if (res.ok || (res.status === 409 && data.alreadyApplied)) {
        ...
        showModal({
          ...
          message: res.ok ? "Your application was submitted successfully." : "You have already applied to this job.",
          type: res.ok ? "success" : "info",
        });
      } else if (data.error?.toLowerCase().includes("profile") || data.error?.toLowerCase().includes("complete")) {
        showModal({
          title: "Complete Your Profile",
          message: data.error || "Please complete your profile before applying.",
          type: "warning",
          primaryLabel: "Go to Profile",
          primaryAction: () => { closeModal(); router.push("/profile"); },
          secondaryLabel: "Cancel",
          secondaryAction: closeModal,
        });
      } else {
        showModal({ title: "Application Failed", message: data.error || "Failed to apply.", type: "error" });
      }
    } catch {
  ...
  const handleInterest = async (jobId) => {
    ...
    try {
      const res = await fetch(
        isInterested ? `/api/interests?jobId=${jobId}` : "/api/interests",
        {
    ...
    <>
      ...
      <div className="min-h-screen pt-24 pb-16 px-6 max-w-7xl mx-auto">
        ...
        <div className="page-header">
          ...
          <p className="page-subtitle">Discover and apply to the latest placement drives at NITS.</p>
        </div>
              ...
              {jobs.map((job, index) => {
                ...
                return (
                  <motion.div
                    ...
                    whileHover={{ y: -4, scale: 1.02, transition: { duration: 0.25, ease: "easeInOut" } }}
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleInterest(job._id); }}
            ...
            >
              <div className="p-8 overflow-y-auto">
                <div className="flex justify-between items-start gap-4 mb-6">
                  ...
                  <button onClick={() => setJobModal(null)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors text-gray-500 shrink-0">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div>
                      <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Deadline</p>
                      <p className="font-medium text-gray-900">
                        {jobModal.deadline ? new Date(jobModal.deadline).toLocaleDateString("en-GB") : "No deadline"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div>
                      <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">Min CGPA</p>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {jobModal.eligibleBranches.map((branch) => (
                        <span key={branch} className="px-2.5 py-1 bg-indigo-50 text-indigo-700 rounded-md text-xs font-medium border border-indigo-100">
                          {branch}
                        </span>
                      ))}
                    </div>
              ...
              <div className="p-6 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 rounded-b-2xl items-center mt-auto">
                {jobModal.jdPdfUrl && (
                  <a href={jobModal.jdPdfUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-indigo-600 font-medium hover:text-indigo-700 px-4 py-2 hover:bg-indigo-50 rounded-lg transition-colors">
                    <ExternalLink className="w-4 h-4" /> View JD
                  </a>
                )}
                <button
                  onClick={() => { handleApply(jobModal._id); setJobModal(null); }}
                ...
                >
                  {appliedJobs.has(jobModal._id) ? <><CheckCircle className="w-4 h-4" /> Applied</> : "Apply Now"}
                </button>
              </div>
            </motion.div>
          </div>
